...
const M = "year", D = "month", T = "day", L = "hour", Y = "minute", H = "second", W = "millisecond", d = {
  ...
  to_string(r, t = !1) {
    ...
    const e = this.get_date_values(r).map((n, a) => (a === 1 && (n = n + 1), a === 6 ? $(n + "", 3, "0") : $(n + "", 2, "0"))), i = `${e[0]}-${e[1]}-${e[2]}`, s = `${e[3]}:${e[4]}:${e[5]}.${e[6]}`;
    return i + (t ? " " + s : "");
  },
  format(r, t = "YYYY-MM-DD HH:mm:ss.SSS", e = "en") {
    const i = new Intl.DateTimeFormat(e, {
      ...
      month: "short"
    }), n = i.format(r), a = n.charAt(0).toUpperCase() + n.slice(1), o = this.get_date_values(r).map((_) => $(_, 2, 0)), h = {
      YYYY: o[0],
      MM: $(+o[1] + 1, 2, 0),
      DD: o[2],
      HH: o[3],
  ...
  diff(r, t, e = "day") {
    ...
    return _ += (r.getDate() - 1) / 31, h = g * 12 + _, r.getDate() < t.getDate() && h--, l = h / 12, e.endsWith("s") || (e += "s"), Math.round(
      {
        milliseconds: i,
        seconds: s,
        minutes: a,
...
class N {
  ...
  draw_bar() {
    ...
    if (this.task.name && this.task.name.toLowerCase().startsWith(t)) {
      ...
      this.$bar = f("rect", {
        ...
        class: "bar",
        append_to: this.bar_group
      }), this.task.color && (this.$bar.style.fill = this.task.color), A(this.$bar, "width", 0, s);
    }
  ...
  calculate_progress_width() {
    ...
    const n = this.x + s, a = this.gantt.config.ignored_positions.reduce((h, l) => h + (l >= this.x && l < n), 0) * this.gantt.config.column_width;
    s += a;
  ...
  draw_label() {
    let t;
    this.is_milestone ? t = this.x + this.height + 5 : (t = this.x + this.$bar.getWidth() / 2, this.task.thumbnail && (t = this.x + this.image_size + 5)), f("text", {
      x: t,
  ...
  update_label_position_on_horizontal_scroll({ x: t, sx: e }) {
    ...
    let o = this.$bar.getX() + this.$bar.getWidth(), h = s.getX() + t, l = n && n.getX() + t || 0, g = n && n.getBBox().width + 7 || 7, _ = h + s.getBBox().width + 7, c = e + i.clientWidth / 2;
    s.classList.contains("big") || (_ < o && t > 0 && _ < c || h - g > this.$bar.getX() && t < 0 && _ > c) && (s.setAttribute("x", h), n && (n.setAttribute("x", l), a.setAttribute("x", l)));
  }
  date_changed() {
    let t = !1;
  ...
  compute_duration() {
    if (["Hour", "Quarter Day", "Half Day"].includes(
      this.gantt.config.view_mode.name
    )) {
      this.duration = d.diff(
        this.task._end,
        this.task._start,
        this.gantt.config.unit
      ) / this.gantt.config.step, this.actual_duration_raw = this.duration, this.ignored_duration_raw = 0, this.task.actual_duration = this.duration, this.task.ignored_duration = 0;
      return;
    }
    let t = 0, e = 0;
  ...
  update_label_position() {
    ...
    if (this.is_milestone) {
      i.classList.add("big"), i.setAttribute("x", this.x + this.height + n);
      return;
    }
    let a = this.image_size + 10;
...
const y = [
  ...
  popup: (r) => {
    ...
    const t = ["Hour", "Quarter Day", "Half Day"].includes(
      r.chart.config.view_mode.name
    ), e = t ? "MMM D HH:mm" : "MMM D", i = d.format(
      r.task._start,
      e,
...
class z {
  ...
  setup_tasks(t) {
    this.tasks = t.map((e, i) => {
      ...
      if (e._start = d.parse(e.start), e.end === void 0 && e.duration !== void 0 && (e.end = e._start, e.duration.split(" ").forEach((h) => {
        let { duration: l, scale: g } = d.parse_duration(h);
        e.end = d.add(e.end, l, g);
      })), !e.end)
      ...
      e._index = i;
      const n = d.get_date_values(e._end), a = [
        "Hour",
        "Quarter Day",
        "Half Day"
      ].includes(this.options.view_mode);
      if (n.slice(3).every((o) => o === 0) && !a && (e._end = d.add(e._end, 24, "hour")), typeof e.dependencies == "string" || !e.dependencies) {
        let o = [];
  ...
  setup_gantt_dates(t) {
    ...
    this.config.date_format = this.config.view_mode.date_format || this.options.date_format, ["Hour", "Quarter Day", "Half Day"].includes(
    ...
    if (t !== null && t !== 21 && (this.$svg.innerHTML = "", this.$container.innerHTML = '<div style="padding:40px;text-align:center;color:#dc2626;font-family:sans-serif;"><h2>⚠️ License Violation Detected</h2><p>This software has been tampered with or is being used without a valid license.</p><p style="margin-top:16px;font-size:14px;color:#666;">If you believe this is an error, please contact support with your purchase details.</p></div>'), t === null) {
      ...
      if (!e) {
        ...
        if (s) {
          ...
          n.className = "header-branding", n.innerHTML = '<a href="https://pythonandvba.com/simplegantt" target="_blank">Made with SimpleGantt</a>', s.appendChild(n);
        }
      }
  ...
  make_side_header() {
    if (this.$side_header = this.create_el({ classes: "side-header" }), this.$upper_header.prepend(this.$side_header), this.options.view_mode_select) {
      ...
      e.selected = !0, e.disabled = !0, e.textContent = x("mode", this.options.language), t.appendChild(e);
    ...
    if (this.options.today_button) {
      ...
      t.classList.add("today-button"), t.textContent = x("today", this.options.language), t.onclick = this.scroll_current.bind(this), this.$side_header.prepend(t), this.$today_button = t;
    }
  }
  make_grid_ticks() {
  ...
  highlight_current() {
    ...
    if (this.$current_highlight = this.create_el({
      ...
      append_to: this.$header
    }), this.options.today_label && !this.$today_label) {
    ...
    } else this.$today_label && (this.$today_label.style.left = n + 6 + "px");
  }
  ...
  create_el({ left: t, top: e, width: i, height: s, id: n, classes: a, append_to: o, type: h }) {
    ...
    return l.style.top = e + "px", l.style.left = t + "px", n && (l.id = n), i && (l.style.width = i + "px"), s && (l.style.height = s + "px"), o && o.appendChild(l), l;
  }
  make_dates() {
  ...
  bind_bar_events() {
    ...
    if (p.on(this.$svg, "mousemove", ".bar-wrapper, .handle", (g) => {
      ...
      _.classList.contains("left") ? (s = !0, _.classList.add("visible")) : _.classList.contains("right") ? (n = !0, _.classList.add("visible")) : _.classList.contains("bar-wrapper") && (t = !0), this.popup && this.popup.hide(), e = g.offsetX || g.layerX, a = c.getAttribute("data-id");
      let u;
